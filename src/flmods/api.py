...
class VoxelWorldAPI:
    ...
    def get_data(self, endpoint, search_query="", sort_method="Популярные", page=1, tag_ids=None):
        ...
        try:
            ...
            log.error("Ошибка API (%s): %s", endpoint, e)
        return []
    ...
    def get_tags(self, tag_type):
        try:
            ...
            log.error("Ошибка получения тегов (%s): %s", tag_type, e)
        return []
    ...
    def get_download_url(self, content_type, content_id, version_id):
        try:
            ...
            log.error("Ошибка получения URL загрузки: %s", e)
        return None
