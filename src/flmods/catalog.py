...
class InstalledSidebar(QWidget):
    def __init__(self, settings_manager, theme_manager=None, parent=None):
        ...
        self.title_label = QLabel(tr("flmods.installed"))
    ...
    def retranslate(self):
        self.title_label.setText(tr("flmods.installed"))
        self.refresh()
...
class ModCard(QFrame):
    ...
    def __init__(self, mod_data, content_type="mods", cached_version=None,
        ...
        if isinstance(author_data, dict):
            ...
            self.author_name = author_data

        self.downloads = mod_data.get('downloads', 0)
        self.raw_date = mod_data.get("last_update_date", "")
        ...
        if self.icon_url and self.icon_url in self.image_cache:
            ...
            self.icon_label.setText(tr("card.image_loading"))
            QTimer.singleShot(0, lambda: self.image_requested.emit(self.icon_url))
        else:
            self.icon_label.setText(tr("card.no_image"))
        ...
        self.action_btn = QPushButton(tr("card.install"))
    ...
    def _update_texts(self):
        self.author_label.setText(tr("card.author", author=self.author_name))
        updated = format_relative_time(self.raw_date)
        self.meta_label.setText(tr("card.downloads_updated", downloads=self.downloads, updated=updated))
        if self.cached_version:
            self.version_text = tr("card.version", version=self.cached_version)
        else:
            self.version_text = tr("card.version_loading")
        self.version_label.setText(self.version_text)

    def retranslate(self):
        if self.icon_url and self.icon_url not in self.image_cache:
            if self.icon_label.text() in (
                "Загрузка...", "Loading...",
            ):
                self.icon_label.setText(tr("card.image_loading"))
            elif self.icon_label.text() in ("No Img",):
                self.icon_label.setText(tr("card.no_image"))
    ...
    def set_image_from_data(self, data):
        if not data:
            self._original_pixmap = None
            self.icon_label.setText(tr("card.image_error"))
            return
        pixmap = QPixmap()
    ...
    def update_version(self, version):
        ...
        self.version_text = tr("card.version", version=version)
        self.version_label.setText(self.version_text)

    def set_installed(self, installed, latest_version=None, installed_version=None):
        if not installed:
            self.action_btn.setText(tr("card.install"))
            self._current_state = "install"
            self._apply_install_style()
            return
        ...
        if has_update:
            self.action_btn.setText(tr("card.update"))
            self._current_state = "update"
            self._apply_update_style()
        else:
            self.action_btn.setText(tr("card.reinstall"))
    ...
    def _on_install(self):
        ...
        prefix_v = tr("card.version", version="")
        for prefix in (prefix_v, "Версия: ", "Version: "):
            if prefix and version_text.startswith(prefix):
                version_text = version_text[len(prefix):]
                break
        self.install_clicked.emit(self.mod_id, self.mod_title,
                                  version_text, self.content_type)
...
class ModsWidget(QWidget):
    ...
    def __init__(self, settings_manager, thread_manager, theme_manager=None, parent=None):
        ...
        self.btn_mods = QPushButton(tr("tab.mods"))
        self.btn_respacks = QPushButton(tr("tab.respacks"))
        self.btn_maps = QPushButton(tr("tab.maps"))
        self.tab_buttons = {
            self.btn_mods: "mods",
        ...
        self.search_bar.setPlaceholderText(tr("search.placeholder"))
        ...
        self.tag_combo.addItem(tr("tags.all"), None)
        self.tag_combo.currentTextChanged.connect(self.on_tag_changed)
        top_layout.addWidget(self.tag_combo)

        self.sort_combo = QComboBox()
        self.sort_combo.addItem(tr("sort.popular"), "popular")
        self.sort_combo.addItem(tr("sort.new"), "new")
        self.sort_combo.addItem(tr("sort.alphabet"), "alphabet")
        self.sort_combo.currentIndexChanged.connect(self.on_sort_changed)
        top_layout.addWidget(self.sort_combo)

        self.btn_refresh = QPushButton(tr("button.refresh"))
    ...
    def retranslate(self):
        self.btn_mods.setText(tr("tab.mods"))
        self.btn_respacks.setText(tr("tab.respacks"))
        self.btn_maps.setText(tr("tab.maps"))
        self.search_bar.setPlaceholderText(tr("search.placeholder"))
        self.btn_refresh.setText(tr("button.refresh"))

        if self.tag_combo.count() > 0:
            self.tag_combo.blockSignals(True)
            self.tag_combo.setItemText(0, tr("tags.all"))
            self.tag_combo.blockSignals(False)

        if self.sort_combo.count() >= 3:
            self.sort_combo.blockSignals(True)
            self.sort_combo.setItemText(0, tr("sort.popular"))
            self.sort_combo.setItemText(1, tr("sort.new"))
            self.sort_combo.setItemText(2, tr("sort.alphabet"))
            self.sort_combo.blockSignals(False)

        if self.installed_sidebar:
            self.installed_sidebar.retranslate()
    ...
    def _get_sort_id(self):
        ...
        return data if data else "popular"
    ...
    def reload_data(self):
        ...
        self.switch_tab(active_btn, self.current_tab, self.search_bar.text(),
                        self._get_sort_id(), tag_id)

    def force_refresh_current_tab(self):
        active_btn = self.get_active_button()
        tag_id = self.tag_combo.currentData()
        self.switch_tab(active_btn, self.current_tab, self.search_bar.text(),
                        self._get_sort_id(), tag_id, force_reload=True)

    def switch_tab(self, clicked_btn, tab_name, search_query="",
                   sort_id="popular", tag_ids=None, force_reload=False):
        t = self._theme()
        topbar_text = t.get("flmods_topbar_text", "white")
        topbar_font_size = int(t.get("flmods_topbar_font_size", 14))
        topbar_padding_v = int(t.get("flmods_topbar_padding_v", 8))
        ...
        cache_key = (tab_name, search_query, sort_id,
        ...
        self.clear_layout()
        self.status_label = QLabel(tr("flmods.loading"))
        ...
        self.worker = APIWorker(tab_name, search_query, sort_id,
    ...
    def load_more_items(self):
        ...
        bottom_label = QLabel(tr("flmods.loading_more"))
        ...
        self.worker = APIWorker(self.current_tab, self.search_bar.text(),
                                self._get_sort_id(), self.current_page,
    ...
    def on_more_data_loaded(self, data):
        ...
        cache_key = (self.current_tab, self.search_bar.text(),
                     self._get_sort_id(), self.tag_combo.currentData())
    ...
    def show_retry_button(self, msg):
        ...
        label = QLabel(tr("flmods.load_error", msg=msg))
        label.setStyleSheet("color: #c0392b; font-weight: bold;")
        label.setWordWrap(True)
        layout.addWidget(label, stretch=1)

        retry_button = QPushButton(tr("flmods.retry"))
    ...
    def apply_filters(self):
        ...
        if not filtered:
            empty_label = QLabel(tr("flmods.not_found"))
