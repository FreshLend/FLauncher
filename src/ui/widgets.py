...
class FullWidthTabWidget(QTabWidget):
    ...
    def _update_style(self, force=False):
        count = max(self.count(), 1)
        w = max(self.width() // count - 3, 60)

        if not force and getattr(self, '_last_w', None) == w and getattr(self, '_last_count', None) == count:
            return
...
class ReleaseDisplayWidget(QWidget):
    ...
    def _build(self):
        ...
        raw = data.get("date_raw", "") or ""
        date_text = format_date(raw) if raw else ""
