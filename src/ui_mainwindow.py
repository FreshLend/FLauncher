...
class FLauncher(QMainWindow):
    ...
    def __init__(self):
        ...
        i18n.load(self.settings_manager.get_language())

        setup_logging(self.settings_manager.logs_path)
        log.info(tr("log.app_start", version=VERSION))
        ...
        self.repo_check_error.connect(lambda msg: self.show_error_message(tr("dialog.error"), msg))
        self.repo_added.connect(self.on_repo_added)
        self.versions_loaded.connect(self._update_versions_combo)
        self.load_failed.connect(lambda msg: self.show_error_message(tr("dialog.data_load_error"), msg))
        ...
        if self.settings.get("discord_rpc_enabled", True):
            ...
            self.set_discord_presence(tr("presence.home"), "")
    ...
    def _connect_signals(self):
        ...
        self.ui.language_selected.connect(self.on_language_selected)
        ...
        self.version_manager.error_occurred.connect(
            lambda msg: self.show_error_message(tr("dialog.error"), msg)
        )
    ...
    def on_update_available(self, latest_version, release_url):
        ...
        msg.setWindowTitle(tr("update.title"))
        msg.setText(tr("update.text", version=latest_version))
        msg.setInformativeText(tr("update.info", current=VERSION))
        update_btn = msg.addButton(tr("update.update"), QMessageBox.ButtonRole.AcceptRole)
        close_btn = msg.addButton(tr("update.close"), QMessageBox.ButtonRole.RejectRole)
    ...
    def on_theme_selected(self, key):
        if self.theme_manager.set_theme(key):
            ...
            self.set_discord_presence(tr("presence.theme_changed"), "", "settings")

    @pyqtSlot(str)
    def on_language_selected(self, code):
        if not code or code == i18n.current():
            return
        self.settings_manager.set_language(code)
        i18n.load(code)
        i18n.set_theme_overrides(
            self.theme_manager.get_text_overrides(code)
        )
        self.ui.retranslate_ui()
        self.update_token_status()
        self.ui.refresh_themes_combo()
    ...
    def set_discord_presence(self, details, state, small_image=None):
        display_state = f"@{self.ui.input_field.text()}" if self.ui.input_field.text() else tr("presence.guest")
        self.discord_rpc.set_presence(details, display_state, small_image)

    def load_all_data(self):
        self.ui.version_combo.clear()
        self.ui.version_combo.addItem(tr("status.getting_versions"))
        self.ui.download_info_label.setText(tr("status.loading_data"))
        ...
        def load_data():
            try:
                ...
                log.exception(tr("log.data_load_error"))
                if self.isVisible():
                    self.load_failed.emit(str(e))

        self.thread_manager.submit(load_data)
    ...
    def _update_versions_combo(self, online_versions_info):
        ...
        if self.ui.version_combo.count() > 1 and self.ui.version_combo.itemText(0) == tr("status.getting_versions"):
    ...
    def _refresh_mods_panel_state(self):
        ...
        if current_version and current_version != tr("status.getting_versions"):
    ...
    def on_play_button_click(self):
        ...
        if selected_version == tr("status.getting_versions"):
            self.show_info_message(tr("dialog.select_version"), tr("dialog.select_version_msg"))
            return
        version_folder = self.version_manager.app_data_path / selected_version
        if not version_folder.exists():
            if self.confirm(
                tr("dialog.version_not_found"),
                tr("dialog.version_not_installed", version=selected_version)
            ):
        ...
        if executable:
            ...
            self.show_error_message(
                tr("dialog.error"),
                tr("dialog.exec_not_found", version=selected_version)
            )
    ...
    def download_version(self, version_tag):
        ...
        self.download_worker.error.connect(
            lambda msg: self.show_error_message(tr("dialog.download_error"), msg)
        )
    ...
    def on_download_finished(self, success, message):
        ...
        self.ui.download_info_label.setText("")
        if success:
            self.show_info_message(tr("dialog.success"), message)
            self.refresh_all_data()
        else:
            self.show_error_message(tr("dialog.error"), message)
    ...
    def cancel_download(self):
        ...
        if self.confirm(tr("dialog.cancel_title"), tr("dialog.cancel_msg"), default_no=True):
            worker.cancel()
            self.ui.download_info_label.setText(tr("dialog.cancel_progress"))
            self.ui.cancel_button.setEnabled(False)

    def launch_game(self, voxel_core_path, working_directory, version):
        try:
            self.set_discord_presence(
                tr("presence.playing", version=version),
                f"@{self.ui.input_field.text()}" if self.ui.input_field.text() else tr("presence.guest"),
                "game"
            )
            ...
            log.exception(tr("log.launch_error"))
            self.show_error_message(tr("dialog.launch_error"), str(e))
            self.set_discord_presence(
                tr("presence.home"),
                f"@{self.ui.input_field.text()}" if self.ui.input_field.text() else tr("presence.guest"),
                "home"
    ...
    def on_flm_button_click(self):
        if self.ui.FL_MODS.isVisible():
            ...
            self.ui.hide_flmods()
            self.set_discord_presence(tr("presence.home"), "", "home")
        else:
            self.ui.show_flmods()
            self.set_discord_presence(tr("presence.flmods"), "", "mods")
    ...
    def toggle_settings(self):
        if self.ui.settings_panel.isVisible():
            ...
            self.set_discord_presence(tr("presence.settings"), "", "settings")
    ...
    def _update_settings_ui(self):
        ...
        if ui.language_combo is not None:
            ui.language_combo.blockSignals(True)
            for i in range(ui.language_combo.count()):
                if ui.language_combo.itemData(i) == current_lang:
                    ui.language_combo.setCurrentIndex(i)
                    break
            ui.language_combo.blockSignals(False)
    ...
    def _build_message_box(self, icon, title, text, informative=""):
        msg = QMessageBox(self)
        msg.setIcon(icon)
    ...
    def show_error_message(self, title, message):
        ...
        msg = self._build_message_box(QMessageBox.Icon.Critical, title, message)
        ok_btn = msg.addButton(tr("dialog.ok"), QMessageBox.ButtonRole.AcceptRole)
        msg.setDefaultButton(ok_btn)
        msg.exec()

    def show_info_message(self, title, message):
        msg = self._build_message_box(QMessageBox.Icon.Information, title, message)
        ok_btn = msg.addButton(tr("dialog.ok"), QMessageBox.ButtonRole.AcceptRole)
        msg.setDefaultButton(ok_btn)
        msg.exec()

    def confirm(self, title, message, yes_key="dialog.yes", no_key="dialog.no", default_no=False):
        msg = self._build_message_box(QMessageBox.Icon.Question, title, message)
        yes_btn = msg.addButton(tr(yes_key), QMessageBox.ButtonRole.YesRole)
        no_btn = msg.addButton(tr(no_key), QMessageBox.ButtonRole.NoRole)
        if default_no:
    ...
    def check_github_token(self):
        ...
        if not token:
            self.ui.update_token_status(tr("token.no_token"), "#f44336")
            return
        ...
        if user:
            ...
            if rate_limit:
                ...
                self.ui.update_token_status(
                    tr("token.valid_full", user=user.get('login', 'Unknown'),
                       remaining=remaining, limit=limit),
                    "#4CAF50"
                )
                if self.confirm(tr("token.refresh_title"), tr("token.refresh_msg")):
                    self.refresh_releases_panel()
            else:
                self.ui.update_token_status(
                    tr("token.valid", user=user.get('login', 'Unknown')),
                    "#4CAF50"
                )
        else:
            self.ui.update_token_status(tr("token.invalid"), "#f44336")

    def update_token_status(self):
        token = self.settings_manager.get_github_token()
        if token:
            self.ui.update_token_status(tr("token.saved"), "#FF9800")
        else:
            self.ui.update_token_status(tr("token.none"), "#9E9E9E")

    @pyqtSlot()
    def refresh_releases_panel(self):
        self.ui.download_info_label.setText(tr("status.loading_releases"))

        def load_releases():
            releases = self.github_client.get_releases_for_display()

        self.thread_manager.submit(load_releases)

    @pyqtSlot()
    def add_github_repository(self):
        repo, ok = QInputDialog.getText(
            self,
            tr("dialog.add_repo_title"),
            tr("dialog.add_repo_msg")
        )
        if ok and repo:
            if len(repo.split('/')) != 2:
                self.show_error_message(tr("dialog.error"), tr("dialog.invalid_format"))
                return
            ...
            def check_and_add():
                try:
                    ...
                    if exists:
                        ...
                        self.repo_check_error.emit(tr("dialog.repo_not_found", repo=repo))
                except Exception as e:
                    self.repo_check_error.emit(tr("dialog.repo_check_error", error=str(e)))
    ...
    def on_mod_install_requested(self, content_id, content_name, version_number, content_type):
        ...
        if not target_version or target_version == tr("status.getting_versions"):
            self.show_error_message(tr("dialog.error"), tr("dialog.select_core_version"))
            return
        version_folder = self.version_manager.app_data_path / target_version
        if not version_folder.exists():
            self.show_error_message(
                tr("dialog.error"),
                tr("dialog.core_not_installed", version=target_version)
            )
            return

        self.ui.download_info_label.setText(tr("dialog.install_progress", name=content_name))
        self.ui.progress_bar.setValue(0)
        self.ui.set_cancel_button_visible(True)
    ...
    def _on_install_finished(self, success, message, content_name, core_version):
        ...
        if success:
            ...
            self.ui.update_mods_installed_status(installed)
            self.show_info_message(tr("dialog.success_title"), message)
        else:
            self.show_error_message(tr("dialog.install_error"), message)
    ...
    def on_version_selected_for_mods(self, version):
        if version and version != tr("status.getting_versions"):
