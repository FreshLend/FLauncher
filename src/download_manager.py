...
from utils import get_platform_asset_pattern, get_executable_pattern, MAIN_REPO, safe_extract_zip
from i18n import tr

log = logging.getLogger("flauncher.download_manager")
...
class DownloadManager:
    ...
    def _download_release(self, version_tag, repo, progress_callback=None, is_cancelled_callback=None):
        try:
            ...
            if response.status_code != 200:
                return False, tr("dl.release_info_failed", repo=repo, code=response.status_code)
            ...
            if not file_url:
                return False, tr("dl.file_missing", version=version_tag, repo=repo)
            ...
            if free_space < required_space and asset_size > 0:
                ...
                return False, tr("dl.no_space", required=f"{required_mb:.0f}", free=f"{free_mb:.0f}")

            zip_path = downloads_path / asset_name

            if progress_callback:
                progress_callback(0, tr("dl.preparing", repo=repo))

            success = self._download_file(file_url, zip_path, asset_size, progress_callback, is_cancelled_callback)
            if not success:
                if zip_path.exists():
                    zip_path.unlink()
                return False, tr("dl.cancelled")

            if is_cancelled_callback and is_cancelled_callback():
                return False, tr("dl.cancelled")

            if progress_callback:
                progress_callback(0, tr("dl.unpacking"))
            ...
            if is_cancelled_callback and is_cancelled_callback():
                ...
                if extraction_path.exists():
                    shutil.rmtree(extraction_path, ignore_errors=True)
                return False, tr("dl.install_cancelled")

            return True, tr("dl.success", version=version_tag)

        except Exception as e:
            log.exception("Download error")
            return False, tr("dl.error", error=e)
    ...
    def _download_artifact(self, version_tag, artifact_data, repo, progress_callback=None, is_cancelled_callback=None):
        try:
            ...
            if response.status_code != 200:
                return False, tr("dl.artifact_failed", repo=repo, code=response.status_code)
                ...
                for data in response.iter_content(chunk_size=chunk_size):
                    ...
                    if data:
                        ...
                        if progress_callback and total_size > 0:
                            ...
                            progress_callback(progress, tr("dl.artifact_download", repo=repo, percent=progress))

            version_folder = self.version_manager.app_data_path / version_tag
            version_folder.mkdir(parents=True, exist_ok=True)

            safe_extract_zip(file_path, version_folder)
            file_path.unlink()

            self.version_manager.create_config_json(version_folder)

            return True, tr("dl.artifact_success", version=version_tag)

        except Exception as e:
            log.exception("Artifact download error")
            return False, tr("dl.artifact_error", repo=repo, error=e)
    ...
    def _download_file(self, file_url, file_path, expected_size, progress_callback=None, is_cancelled_callback=None):
        try:
                    ...
                    if data:
                        ...
                        if progress_callback and total_size > 0:
                            ...
                            progress_callback(
                                ...
                                f"{speed:.1f} KB/s, {remaining_time_str}"
                            )
            ...
            log.error(tr("log.download_network_error", error=e))
            return False
        except Exception:
            log.exception(tr("log.download_file_error"))
            return False
    ...
    def _extract_archive(self, archive_path, version_tag, is_cancelled_callback=None):
        ...
        if file_ext == '.zip':
            with zipfile.ZipFile(archive_path, 'r') as zip_ref:
                ...
                if bad_file:
                    raise Exception(tr("dl.corrupted", file=bad_file))

            safe_extract_zip(archive_path, extraction_path)
                ...
                for item in folder_path.iterdir():
                    ...
                    if dest.exists():
                        if dest.is_dir():
                            ...
                            dest.unlink()
                    shutil.move(str(item), str(extraction_path))
            ...
            try:
                ...
                raise Exception(tr("dl.dmg_mount_error", error=e))
            except Exception as e:
                raise Exception(tr("dl.dmg_unpack_error", error=e))
        else:
