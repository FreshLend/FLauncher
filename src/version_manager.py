...
class VersionManager(QObject):
    ...
    def shutdown(self):
        ...
        self.progress.emit(tr("vm.shutdown"))
    ...
    def get_github_repo_versions(self, repo):
        ...
        try:
            ...
            if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                self.error_occurred.emit(tr("vm.versions_error", repo=repo, error=str(e)))
            return []
    ...
    def _get_workflow_artifacts(self, repo, workflow_file, build_name, pattern, build_type, limit=50):
        ...
        try:
            ...
            if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                self.error_occurred.emit(tr("vm.artifacts_workflow_error",
                                            workflow=workflow_file, repo=repo, error=e))
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if artifacts and not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                    ...
                    self.progress.emit(tr("vm.loaded_artifacts", n=len(artifacts[:max_count]), repo=repo))
            except Exception as e:
                if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                    self.error_occurred.emit(tr("vm.artifacts_error", repo=repo, error=e))
    ...
    def get_all_online_versions(self):
        ...
        self.progress.emit(tr("vm.loading_versions", n=len(repos_to_check)))
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                    self.error_occurred.emit(tr("vm.repo_error", error=str(e)))
           ...
           self.settings_manager.settings["artifacts"]["enabled"]:
            ...
            for future in as_completed(artifact_futures):
                ...
                try:
                    ...
                    if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                        self.error_occurred.emit(tr("vm.artifacts_generic_error", error=e))

        if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
            all_versions.sort(key=lambda x: x.get('date', ''), reverse=True)
            self.progress.emit(tr("vm.total_loaded", n=len(all_versions)))
    ...
    def get_username_from_config(self, version):
        ...
        try:
            ...
            if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                self.error_occurred.emit(tr("vm.config_read_error", error=e))
    ...
    def update_username_in_config(self, username, version):
        ...
        try:
            ...
            if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                self.error_occurred.emit(tr("vm.config_write_error", error=e))
    ...
    def create_config_json(self, extraction_path):
        ...
        try:
            ...
            if not (self._is_shutting_down or self.thread_manager.is_shutting_down()):
                self.error_occurred.emit(tr("vm.config_create_error", error=e))