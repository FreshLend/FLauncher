...
from utils import (
    MAIN_REPO, get_platform_asset_pattern, get_platform_name,
)
from i18n import tr, format_date
...
class GitHubClient(QObject):
    ...
    def get(self, url, params=None, timeout=10):
        try:
            ...
            self.error_occurred.emit(tr("gh.api_error", error=e))
            return None
    ...
    def check_rate_limit(self):
        try:
            ...
            self.error_occurred.emit(tr("gh.rate_limit_error", error=e))
        return None
    ...
    def get_authenticated_user(self):
        try:
            ...
            if response and response.status_code == 200:
                ...
                self.error_occurred.emit(tr("gh.token_invalid"))
            elif response and response.status_code == 403:
                self.error_occurred.emit(tr("gh.rate_limit_reached"))
        except Exception as e:
            self.error_occurred.emit(tr("gh.token_check_error", error=e))
        return None
    ...
    def get_releases_for_display(self, limit=10):
        ...
        try:
            ...
            if not response or response.status_code != 200:
                self.error_occurred.emit(tr("gh.releases_error"))
                return []
            ...
            for release in releases[:limit]:
                if isinstance(release, dict):
                    ...
                    release_date_formatted = format_date(release_date_str) if release_date_str else ""
            ...
            self.error_occurred.emit(tr("gh.releases_network_error"))
            return []
        except Exception as e:
            self.error_occurred.emit(tr("gh.data_error", error=e))
            return []